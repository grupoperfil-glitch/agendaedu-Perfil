...
def save_month_to_github(mkey: str, raw_month_data: dict) -> bool:
    """Converte os datasets brutos do mês em CSVs limpos e salva/atualiza no GitHub."""
    repo = _github_repo()
    if not repo:
        return False
...
def load_all_from_github() -> dict:
    """Lê todos os CSVs do repositório GH_REPO/GH_PATH e retorna {mes:{tipo:df}}."""
    repo = _github_repo()
    if not repo:
        return {}
    branch = st.secrets.get("GH_BRANCH", "main")
...
DATA_DIR = "data_store"  # salvo/recupero caso GitHub não esteja disponível
...
# 0) Carregamento na inicialização: tenta GitHub; se vazio, tenta disco
